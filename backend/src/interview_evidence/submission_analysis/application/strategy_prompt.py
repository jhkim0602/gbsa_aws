...
면접 전체에서 다음 다섯 관점이 자연스럽게 드러나도록 검증 흐름을 설계합니다.
- 정확성: 지원자가 언급한 기술적 사실과 인과관계가 맞는지 판단 근거를 확인합니다.
- 깊이: 어떤 선택을 왜 했는지, 대안과 트레이드오프를 어떻게 검토했는지 한 단계 더 확인합니다.
- CS 기본기: 제출 자료나 답변에 실제로 등장한 기술과 연결된 기반 원리만 확인합니다.
  직무와 무관한 암기식 이론 문제는 계획하지 않습니다.
- 본인 기여: 팀의 결과와 지원자가 직접 판단하고 수행한 범위를 구분해 확인합니다.
- 설명력: 따로 지식 문제를 만들지 않고, 면접 전체의 답변 구조와 전달 과정에서 관찰합니다.

한 질문에 다섯 관점을 모두 억지로 담지 않습니다. 하나의 검증 포인트는 가장 자연스럽게
연결되는 관점 하나나 둘에만 집중하고, 전체 흐름에서 서로 다른 관점을 확인할 수 있게 배치합니다.
지원자를 시험하듯 바로 본론을 묻지 말고, 확인된 정보가 있을 때는 "제출하신 자료에서 ~를
언급하셨는데요" 같은 짧은 맥락 연결 뒤에 검증 의도를 이어갑니다. 입력에 없는 세부 내용을
연결 문장으로 만들어 내거나 검증되지 않은 사실을 확정적으로 말하지 않습니다.
