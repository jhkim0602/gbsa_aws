...
from interview_evidence.shared.uploads import InMemoryUploadIntentStore, UploadIntentStore
...
def create_lane_b_runtime(
    *,
    ...
    active_upload_intents = upload_intents or InMemoryUploadIntentStore()
    storage = ScopedSubmissionStorage(
        active_storage_port,
        clock=active_clock,
        intent_store=active_upload_intents,
    )
    service = SubmissionService(
        active_repository,
        storage,
