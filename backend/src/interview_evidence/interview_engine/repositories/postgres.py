...
class InMemoryInterviewRepository:
    def __init__(self) -> None:
        self.equipment_checks: dict[UUID, EquipmentCheck] = {}
        self.sessions: dict[UUID, InterviewSession] = {}
        self.turns: dict[UUID, InterviewTurn] = {}
    ...
    def save_session(self, context: TenantContext, session: InterviewSession) -> InterviewSession:
        require_tenant_context(context).assert_company(session.company_id)
        self.sessions[session.interview_session_id] = session
        return session
    ...
    def save_equipment_check(self, context: TenantContext, check: EquipmentCheck) -> EquipmentCheck:
        ...
        self.equipment_checks[check.equipment_check_id] = check
        return check

    def get_equipment_check(
        self, context: TenantContext, equipment_check_id: UUID
    ) -> EquipmentCheck:
        return self._scoped(context, self.equipment_checks, equipment_check_id)
    ...
    def save_turn(self, context: TenantContext, turn: InterviewTurn) -> InterviewTurn:
        ...
        for existing in self.turns.values():
            if (
                existing.company_id == turn.company_id
                and existing.interview_session_id == turn.interview_session_id
                and existing.sequence == turn.sequence
                and existing.turn_id != turn.turn_id
            ):
                raise ValueError("turn sequence already exists")
            if (
                existing.company_id == turn.company_id
                and existing.interview_session_id == turn.interview_session_id
                and existing.idempotency_key == turn.idempotency_key
                and existing.turn_id != turn.turn_id
            ):
                raise ValueError("turn idempotency key already exists")
        self.turns[turn.turn_id] = turn
        return turn

    def get_turn(self, context: TenantContext, turn_id: UUID) -> InterviewTurn:
        return self._scoped(context, self.turns, turn_id)
        ...
        return tuple(
            sorted(
                (
                    ...
                    if turn.company_id == tenant.company_id
                    and turn.interview_session_id == session_id
                ),
    ...
    def latest_checkpoint(
        ...
        candidates = (
            ...
            if checkpoint.company_id == tenant.company_id
            and checkpoint.interview_session_id == session_id
        )
        return max(
            candidates,
            key=lambda checkpoint: (checkpoint.session_sequence, checkpoint.created_at),
            default=None,
        )
    ...
    def save_recording_chunk(self, context: TenantContext, chunk: RecordingChunk) -> RecordingChunk:
        ...
        for existing in self.recording_chunks.values():
            if (
                existing.company_id == chunk.company_id
                and existing.interview_session_id == chunk.interview_session_id
                and existing.sequence == chunk.sequence
                and existing.recording_chunk_id != chunk.recording_chunk_id
            ):
                raise ValueError("recording chunk sequence already exists")
            if (
                existing.company_id == chunk.company_id
                and existing.interview_session_id == chunk.interview_session_id
                and existing.idempotency_key == chunk.idempotency_key
                and existing.recording_chunk_id != chunk.recording_chunk_id
            ):
                raise ValueError("recording idempotency key already exists")
        self.recording_chunks[chunk.recording_chunk_id] = chunk
        return chunk
    ...
    def list_recording_chunks(
        ...
        return tuple(
            sorted(
                (
                    chunk
                    for chunk in self.recording_chunks.values()
                    if chunk.company_id == tenant.company_id
                    and chunk.interview_session_id == session_id
                ),
    ...
    def save_question_source_references(
        ...
        for reference in references:
            ...
            turn = self.get_turn(context, reference.question_turn_id)
            if turn.interview_session_id != reference.interview_session_id:
                raise ValueError("question source reference session mismatch")
            self.question_source_references[reference.source_reference_id] = reference
        return references
    ...
    def list_question_source_references(
        ...
        *,
        ...
        return tuple(
            reference
            for reference in self.question_source_references.values()
            if reference.company_id == tenant.company_id
            and reference.question_turn_id == question_turn_id
        )
