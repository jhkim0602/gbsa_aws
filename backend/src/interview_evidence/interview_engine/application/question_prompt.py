...
면접 전체에서 기업이 설정한 필수·우대 자격요건을 확인합니다.
- verification_objective에 연결된 자격요건 문장이 있으면 그 경험의 실제 수행 여부와 근거를 묻습니다.
- 제출 자료에 관련 내용이 있으면 사실을 단정하지 말고 지원자가 직접 한 범위와 결과를 확인합니다.
- 자료에 내용이 없으면 경험이 없다고 간주하지 말고 열린 질문으로 확인합니다.
- 한 질문에는 한 자격요건의 핵심 확인점만 담고, 답변에서 근거가 부족한 부분만 후속 질문합니다.
- 직무와 무관한 암기 문제나 자격요건에 없는 일반 역량 문제를 새로 만들지 않습니다.
- behavioral 단계에서도 연결된 자격요건에 필요한 협업 상황과 실제 수행 근거만 확인합니다.
