...
## Phase 32: Playable Recording

**Purpose**: Make the recording the report cites actually play. The reviewer's whole workflow is to
read a claim and jump to the moment that supports it, and every layer of that path passed its own
tests while the reviewer got a placeholder: the playback locator returned a hostname that resolves
nowhere, the media worker wrote an asset naming an object no code had produced, and the local seed
declared that asset ready without uploading any bytes. Three separate defects, one symptom, zero
failing tests — because no test ever fetched the URL it was handed. The lesson: an endpoint that
returns a URL is only covered by a test that follows it.

- [X] T306 Lane D — Sign the playback URL against the object that holds the bytes. `ScopedPlaybackLocator` returned a hardcoded `https://media.local/playback`, then — once it took a presigner — took it as an optional argument that all three composition roots omitted, so the endpoint answered `status: ready` with `url: null`. A `RecordingPresigner` protocol, `AwsS3ObjectStorage.create_playback_url` and the wiring in `runtime/production.py` and `main.py` replace it; the arg-less construction inside `create_lane_d_runtime` stays as the fallback the class documents, since a lane runtime built without a bucket has nothing to sign. The 5-minute TTL is unchanged, and no signed URL is logged (FR-037, FR-038, FR-049, FR-057)
- [X] T307 Lane D — Produce the object the asset names. `RecordingAssembler` concatenates the verified chunks in sequence order into one `.webm` and returns the key it wrote, replacing the `.m3u8` key the worker had fabricated for a manifest nothing generated. No transcode: `MediaRecorder` timeslices concatenated in order *are* the original stream, which is why this runs in the worker rather than through a managed media service. `assembled_recording_key()` is the single definition of that layout, shared by the worker and the local seed — a second copy of the f-string is precisely how the review screen came to ask the bucket for a key nothing had written. Two keys exist by design: transcripts cite the chunk, the reviewer plays the assembly (FR-037, QG-13-QG-15)
- [X] T308 Integration — Upload real bytes in the local seed. `seed_local_company` takes an optional `media_storage`, and `runtime/aws.create_media_object_storage` gives the compose seed a media-bucket adapter without building the rest of the runtime. Without a bucket the asset now stays `processing` instead of claiming to be playable, because an asset marked ready is a promise that playback can sign a URL for bytes that are there. The seeded chunk row carries the real digest and length of the uploaded bytes rather than a stand-in pair, which had made `verify_uploaded_object` reject the very object the seed wrote. `runtime/demo_recording.webm` runs the full 160s of the seeded timeline so every citation seeks to a frame that exists, and ships in the wheel (FR-030-FR-034, QG-04)
- [X] T309 Integration — Follow the URL, in `backend/tests/integration/test_timeline_playback_http.py` and `test_local_demo_recording_bytes.py`. The HTTP test drives `create_production_runtime` through positions → invitations → timeline and asserts the URL is non-null, names an object present in the same store the runtime signed against, and holds bytes starting with the EBML magic — a browser decides playability from the container header, so a placeholder of the right length still leaves `<video>` at `readyState 0`. Mutation-verified: reverting the presigner wiring fails it with `assert 'unavailable' == 'ready'`. `test_local_review_seed.py` was itself wrong and is fixed — it asserted `ready` while seeding with no bucket, encoding the defect (QG-04, QG-13-QG-15)
- [X] T310 Integration — Assert the browser plays it, in `tests/browser/company-console.spec.ts`, with nothing mocked. The test walks the seeded review path, fetches the same signed URL the console put in `<video src>` and requires a 200 with `video/webm`, then reads `readyState`, `videoWidth` and `duration` back out of the element and seeks to a cited 120s offset. Mutation-verified: repointing `src` at a nonexistent key makes the browser fire `error` and the test fail. The existing `playback: { url: null }` mock is left alone — it belongs to the print test's synthetic session, which genuinely has no recording (FR-038, FR-053-FR-055, FR-057)
- [ ] T311 Lane C — Decide the fate of the MediaConvert adapter, now that assembly needs no transcode. `AwsMediaConvert`, `MediaConvertPort`, `MediaConvertClient`, `LocalMediaConvert` and the `media_convert` field on the runtime bundle are constructed by both runtimes and called by nothing: no `create_job` caller exists outside the adapter's own unit test, while `infra/modules/compute` still provisions an IAM role and passes its ARN to the task. Harmless as it stands, but it reads as a transcode pipeline the platform does not have, and the role is a real grant with no consumer. Either give it a consumer or remove adapter, port, wiring and role together; recorded rather than deleted, since neither is this session's to remove (same treatment as T301)
